...
| `LLM_SAMPLE_RATE` | **Updated 2026-09-03 (`docs/DEMO_READINESS.md` Unit AI): this row's "real-world value" is now also the demo's.** Routing is by ambiguity, `LLM_ROUTE_CONFIDENCE_THRESHOLD=0.80` (`configs/demo.env`): a live model is only offered to a record whose rules-engine `Confidence` is below that. `LLM_SAMPLE_RATE=0.15` remains, reinterpreted as a running-ratio ceiling on total live calls (a ceiling, never a selector), not the old hash-of-`record_id` sample this row used to describe. | route by ambiguity, call the model only when the deterministic table is not confident, bounded by a ceiling on total calls | free-tier rate limits (30 RPM Groq, 10-15 RPM Gemini) do not fit a 50-to-100-record demo batch at all, so a ceiling still has to exist; the remaining simplification is the ceiling itself, a running ratio rather than a full per-batch rank, chosen because the Decision Engine classifies `raw.events` as a stream and never sees the whole batch before deciding (`docs/DECISIONS.md` 2026-09-03) |
