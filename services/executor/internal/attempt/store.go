...
//
// messageSource records which rung composed message (SOURCE_LLM or
// SOURCE_RULES_FALLBACK), SOURCE_UNSPECIFIED for a retry, so the audit
// trail can distinguish a generated message from a templated one
...
func (s *Store) Claim(ctx context.Context, recordID string, attemptNumber int32, action commonv1.ActionType, message string, messageSource commonv1.Source, evScoreAtDecision, pRecoveryAtDecision float64, now time.Time) (id string, claimed bool, err error) {
	id = uuid.NewString()
	_, err = s.pool.Exec(ctx, `
		INSERT INTO intervention_attempt
			(id, record_id, attempt_number, action_type, outcome, executed_at, cost_paise, message_text, message_source, failure_code, ev_score_at_decision, p_recovery_at_decision)
		VALUES ($1, $2, $3, $4, $5, $6, 0, $7, $8, '', $9, $10)`,
		id, recordID, attemptNumber, action.String(), claimMarker, now, message, messageSource.String(), evScoreAtDecision, pRecoveryAtDecision)
	if err == nil {
		return id, true, nil
	}
	switch {
